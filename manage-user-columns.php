...
if (!function_exists('dpk_muc_init')) {
	function dpk_muc_init(){
		if ( is_admin() ){
			add_action( 'admin_enqueue_scripts', 'srm_muc_styles_scripts' );
			add_filter( 'manage_users_columns', 'srm_muc_modify_user_table' );
			add_filter( 'manage_users_custom_column', 'srm_muc_modify_user_table_row', 10, 3 );
			add_filter( 'manage_users_sortable_columns', 'srm_muc_allow_sorting' );
			add_action('load-users.php', 'srm_muc_users_page_loaded');
			add_action( 'admin_footer', 'srm_muc_users_page_html' );
			add_action( 'pre_get_users', 'srm_muc_pre_sorting' );
		}
	}
}

if (!function_exists('srm_muc_styles_scripts')) {
	function srm_muc_styles_scripts(){
		wp_register_style( 'srm-muc-style', plugins_url('style.css',__FILE__ ) );
		wp_enqueue_style( 'srm-muc-style' );
...
if (!function_exists('srm_muc_modify_user_table')) {
	function srm_muc_modify_user_table( $columns ) {
...
if (!function_exists('srm_muc_modify_user_table_row')) {
	function srm_muc_modify_user_table_row( $output, $column_id, $uid ) {
...
if (!function_exists('srm_muc_allow_sorting')) {
	function srm_muc_allow_sorting( $columns ) {
		$muc_cc = get_option('muc_custom_cols');
...
// Fixed bugs - sorting
if (!function_exists('srm_muc_pre_sorting')) {
	function srm_muc_pre_sorting( $query ) {
...
if (!function_exists('srm_muc_users_page_loaded')) {
	function srm_muc_users_page_loaded() {
